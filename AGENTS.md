...
- `aws.py` — all AWS interaction, encapsulated in the `AWS` class (S3 upload/delete/list, CloudFront invalidation, IAM policy generation). Keep boto3 calls isolated here; don't scatter `boto3.client(...)` elsewhere. `invalidate_cache()` is a generator that yields status while polling.
- `config.py` — TOML config handling. Config lives at `PlatformDirs("sobe").user_config_path / "config.toml"`. On first run (or if `aws.bucket` is still the placeholder `example-bucket`), `load_config()` writes `DEFAULT_TEMPLATE` and raises `MustEditConfig`; `main()` catches it, prints the path, and exits 1.

Flow: `main()` loads config → parses args → constructs `AWS(config.aws)` → dispatches on flags (`--policy`, `--list`, upload/`--delete`, then optional `--invalidate`).

The flag-combination validation in `parse_args()` is intricate (which flags require/exclude files, `--remote-name` single-file rule, etc.) — read it before touching CLI behavior. A 1.0 release spec in `specs/1.0-release.md` treats the CLI surface as a compatibility contract and tracks open decisions (e.g. `--year` naming, type checker choice).
