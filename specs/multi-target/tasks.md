...
- [x] Step 1: Add the new data model: `ConfigError`, `StorageConfig`,
      ...
      **replaces** the legacy `Config` (same name — they cannot coexist;
      ...
      the replaced `Config`).
- [x] Step 2: Implement `Config.select(name)` with the four selection rules
      (named, default marker, single implicit, ambiguous-multiple) and their
      `ConfigError` messages listing defined target names.
- [x] Step 3: Replace `DEFAULT_TEMPLATE` with the new-schema template and add
      the unconfigured test (zero targets, or every bucket still
      `example-bucket`).
- [x] Step 4: Add the private TOML emitter for the new schema: scalar values
...
- [x] Step 5: Add old-schema detection (top-level `aws`, no `target`) and
...
- [x] Step 6: Rewire `load_config() -> tuple[Config, Migration | None]`:
...
- [x] Step 7: Tests for the model: default + custom values for every new key,
      each validation error (missing bucket, bad provider type, bad name,
      non-scalar `aws_session` value), URL normalization.
- [x] Step 8: Tests for `Config.select`: all four rules plus unknown-name and
      bad-default errors, asserting the messages list defined names.
- [x] Step 9: Tests for template/unconfigured/migration/load: new-schema load,
...
- [x] Step 10: Rework `AWS` to consume `Target`: CloudFront client only
...
- [x] Step 13: Rework `main()`: unpack `(config, migration)` and print the
...
- [x] Step 14: Delete the legacy `AWSConfig` class from
...
- [x] Step 15: Tests for `parse_args()` target rules: with each operation,
...
- [x] Step 16: Tests for `main()` flows: migration notice text, both
...
- [x] Step 17: Rewrite `docs/configuration.md`: new template, every key
...
- [x] Step 18: Update `docs/usage.md`: `-t/--target` examples (and a note
...
- [x] Step 19: Update `README.md`: replace the now-false old-schema TOML
...
- [x] Step 19b: Update `CLAUDE.md` architecture notes: the `config.py`
...
- [x] Step 19c: Reconcile `specs/1.0-release.md` item 2 (first-run wizard)
